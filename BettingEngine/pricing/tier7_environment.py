# pricing/tier7_environment.py
# Tier 7 — Environmental layer.
#
# Sub-layer A: Weather
#   Rain and wind suppress scoring (totals only — no handicap).
#   Dew at night also suppresses scoring.
#
# Sub-layer B: Lunar phase (experimental, off by default in V1)
#   Configurable via tiers.yaml tier7_environment.lunar.enabled.
#
# This module computes deltas from a weather_conditions row already fetched
# and stored in the DB.  It does NOT call any external API — that is the
# responsibility of scripts/fetch_weather.py.
#
# OpenClaw note: fetch_weather.py's fetch_weather_for_match() is the
# integration seam.  Replace that function with an agent call to swap
# data sources without touching the pricing logic here.
...
# Delta table — condition_type → totals_delta
...
TOTALS_DELTA_CAP = -6.0   # floor — totals_delta can never go below this
...
def classify_condition(
    ...
    """
    Classify the match weather into a condition_type and return its totals_delta.

    Priority order (most severe evaluated first so the worst condition wins):
...
def compute_dew_risk(kickoff_datetime: str, temp_c: float, dew_point_c: float) -> bool:
    ...
    kickoff_datetime: ISO string, e.g. '2026-04-10T19:50:00'
    """
...
# ---------------------------------------------------------------------------
# Main entry point — used by price_round.py
# ---------------------------------------------------------------------------
...
def compute_weather_adjustments(
    ...
    """
    Compute Tier 7A weather adjustments from a weather_conditions DB row.

    Args:
        weather_row:       Row from weather_conditions (may be None if not fetched).
        kickoff_datetime:  ISO local datetime string from matches.kickoff_datetime.
        config:            tiers.yaml['tier7_environment'] dict.
    ...
    if not cfg.get('enabled', True):
        return {
            ...
            'dew_risk':       False,
            '_debug': {'reason': 'tier7 weather disabled'},
        }
    ...
    # Use stored dew_risk if present; recompute as fallback
...
# ---------------------------------------------------------------------------
# Lunar sub-layer (Tier 7B) — experimental, off by default
# ---------------------------------------------------------------------------

# ---------------------------------------------------------------------------
# Compatibility shim for legacy engine.py import
# ---------------------------------------------------------------------------

def compute_environment_adjustments(match: dict, context: dict, config: dict) -> list:
    """
    Compatibility shim — engine.py still imports this name.
    The live pricing path (price_round.py / prepare_round.py) calls
    compute_weather_adjustments() directly using data from weather_conditions.
    This shim returns an empty list so engine.py continues to work.
    """
    return []
...
def compute_lunar_adjustments(
    ...
    """
    Compute Tier 7B lunar phase adjustments.
    Only runs if tiers.yaml lunar.enabled is True.
