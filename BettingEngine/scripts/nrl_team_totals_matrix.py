...
SOURCE_PATH = "/Users/elliotbladen/Downloads/nrl (2).xlsx"
OUTPUT_PATH = "/Users/elliotbladen/Betting_model/outputs/nrl_team_totals_matrix.xlsx"
...
def build_moon_sets(start_date: date, end_date: date):
    """Return (new_moon_dates set, full_moon_dates set) covering start–end with ±1 day window."""
    new_moons = set()
    full_moons = set()
...
# Map Excel team names → canonical display names
TEAM_NAME_MAP = {
...
def load_data():
    ...
    for raw in ws.iter_rows(min_row=3, max_row=ws.max_row, values_only=True):
        # Col indices (0-based): A=0, B=1, C=2, D=3, E=4, F=5, G=6, H=7, AO=40
        game_date_raw = raw[0]
        kickoff_raw = raw[1]
        home_team_raw = raw[2]
        away_team_raw = raw[3]
        ...
        market_total_close = raw[40]   # Total Score Close (AO)
...
def enrich_rows(rows):
    """Add moon flags, time/day flags, rest days, and prev-result per team."""
    ...
    # Per-team rest days and previous result
...
def build_team_sheet(wb, team, all_rows, all_teams, all_venues):
...
def main():
    ...
    print("Enriching (moon phases, rest, form)...")
    all_rows = enrich_rows(all_rows)

    all_teams = sorted(set(r["home_team"] for r in all_rows) | set(r["away_team"] for r in all_rows))
    all_venues = sorted(set(r["venue"] for r in all_rows))
    print(f"  {len(all_teams)} teams, {len(all_venues)} venues")
