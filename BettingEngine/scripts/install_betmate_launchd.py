...
def main() -> None:
    parser = argparse.ArgumentParser(description="Install macOS launchd schedule for Betmate auto pricing.")
    parser.add_argument("--config", default="config/betmate_automation.yaml")
    parser.add_argument("--hour", type=int, default=19)
    parser.add_argument("--minute", type=int, default=3)
    ...
    logs_dir = ROOT / "logs" / "betmate"
    logs_dir.mkdir(parents=True, exist_ok=True)

    program_args = [
        sys.executable,
        str(ROOT / "scripts" / "betmate_auto_price.py"),
        "--config",
        args.config,
    ]
    ...
    print(f"Schedule: Monday {args.hour:02d}:{args.minute:02d}")
