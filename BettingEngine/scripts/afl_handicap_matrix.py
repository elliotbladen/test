"""
AFL Team Handicap Matrix Builder
Generates one sheet per AFL team showing handicap cover rate vs market implied 50%.
Source: /Users/elliotbladen/Downloads/afl (2) (1).xlsx (seasons 2022–2025)
Output: outputs/afl_handicap_matrix.xlsx
        outputs/afl_handicap_matrix.csv

COLUMNS
-------
  Cover Rate %          — % of games team covered the spread
...
AFL favourite thresholds (line = 3 goals = 18 pts):
  Heavy Fav  line ≤ -18.5
  Slight Fav line -1 to -18
  Slight Dog line +1 to +18
  Big Dog    line ≥ +18.5
"""
...
def load_data():
    ...
    for raw in ws.iter_rows(min_row=3, max_row=ws.max_row, values_only=True):
        ...
        #   26 = Home Line Close,  23 = Home Line Open
        hcap_home_close = raw[26] if raw[26] is not None else raw[23]
...
def team_line(game, team):
    """Handicap line from team's perspective. Negative = favoured."""
    if game["home_team"] == team:
...
def build_team_sheet(wb, team, all_rows, all_teams, csv_rows):
    ...
    tc = ws.cell(row=1, column=1, value=f"{team} — AFL Handicap Matrix (2022–2025)")
    ...
    # AFL lines are much bigger — use 18.5 pts (3 goals) as threshold
    ...
    row("Heavy Fav (line ≤ -18.5)",    [x for x in g if team_line(x, team) <= -18.5])
    row("Slight Fav (line -1 to -18)", [x for x in g if -18.0 <= team_line(x, team) <= -1.0], alt=True)
    row("Slight Dog (line +1 to +18)", [x for x in g if 1.0 <= team_line(x, team) <= 18.0])
    row("Big Dog    (line ≥ +18.5)",   [x for x in g if team_line(x, team) >= 18.5], alt=True)
