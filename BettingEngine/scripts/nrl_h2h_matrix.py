"""
NRL Team H2H Matrix Builder
Generates one sheet per NRL team showing actual win % vs market implied win %.
Source: /Users/elliotbladen/Downloads/nrl (5).xlsx (seasons 2022–2025)
Output: outputs/nrl_h2h_matrix.xlsx
"""
...
OUTPUT_PATH = "/Users/elliotbladen/Betting_model/outputs/nrl_h2h_matrix.xlsx"
SEASONS     = (2022, 2023, 2024, 2025)
MIN_SAMPLE  = 3
EDGE_FLAG_PCT  = 15.0   # flag if actual win% deviates from implied by 15%+ (relative)
MOON_WINDOW_DAYS = 1
...
def load_data():
    ...
    for raw in ws.iter_rows(min_row=3, max_row=ws.max_row, values_only=True):
        ...
        # Prefer closing price; fall back to opening price if close is unavailable
        home_odds_close  = raw[16] if raw[16] is not None else raw[13]  # Home Odds Close / Open
        away_odds_close  = raw[20] if raw[20] is not None else raw[17]  # Away Odds Close / Open
...
def team_win(game, team):
    """Did this team win? Returns True/False."""
...
def implied_prob(game, team):
    """Market implied win probability for this team."""
    if game["home_team"] == team:
...
def build_team_sheet(wb, team, all_rows, all_teams):
    ...
    tc = ws.cell(row=1, column=1, value=f"{team} — NRL H2H Matrix (2022–2025)")
